...
private:
	...
	void assignColor(){
		string colors[] = {"Black", "Grey", "White", "Tan", "Ashen", "Blue", "Brown", "Castor"};
		color = colors[rand()%8];
	}
...
public:
	...
	void cull(){
		...
		for(int i=0;i<temptotal/2;i++){
			...
			cout<< temp->next->instance.name<< " has died to the cull!"<<endl;
			node* todelete = temp->next;
			temp->next = temp->next->next;
			delete todelete;
...
void Dungeon_Crawl(){
	...
	while (!done){
		...
		while(1){
			...
			cin>>check;
			switch(check){
				case 's': location[0] = (location[0]+1)<10?location[0]+1:location[0];
				goto DONE;

				case 'a': location[1] = (location[1]-1)>=0?location[1]-1:location[1];
				goto DONE;

				case 'w': location[0] = (location[0]-1)>=0?location[0]-1:location[0];
				goto DONE;

				case 'd': location[1] = (location[1]+1)<10?location[1]+1:location[1];
				goto DONE;
